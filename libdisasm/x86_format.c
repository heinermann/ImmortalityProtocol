...
static void get_operand_data_str( x86_op_t *op, char *str, int len ){
        ...
        if ( op->flags & op_signed ) {
                switch ( op->datatype ) {
                        case op_byte:
                                snprintf( str, len, "%hhd", op->data.sbyte );
                                return;
                        case op_word:
                                snprintf( str, len, "%hd", op->data.sword );
                                return;
                        case op_qword:
                                snprintf( str, len, "%lld", op->data.sqword );
                                return;
                        default:
                                snprintf( str, len, "%d", op->data.sdword );
                                return;
                }
        }
        ...
        switch ( op->datatype ) {
                case op_byte:
                        snprintf( str, len, "0x%02hhX", op->data.byte );
                        return;
                case op_word:
                        snprintf( str, len, "0x%04hX", op->data.word );
                        return;
                case op_qword:
                        snprintf( str, len, "0x%08llX", op->data.sqword );
                        return;
                default:
                        snprintf( str, len, "0x%08X", op->data.dword );
                        return;
        }
}
